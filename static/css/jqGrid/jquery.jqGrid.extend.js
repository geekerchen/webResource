    ...
    var DataGrid = function(options, $this) {
        ...
        options = $.extend({
                        ...
                        if (postData.nodeid) {
                            if (options.expandNodeClearPostData == true) {
                                ...
                                if (typeof options.expandNodeClearPostData == "string") {
                                    $.each(options.expandNodeClearPostData.split(","),
                                    function(idx, val) {
                                        if (val != "") {
                                            ps[val] = ""
                                        }
            ...
            loadComplete: function(data) {
                if(dataGridPage.length >= 1 && data && options.datatype == "local"){
            ...
            onSortCol: function(index, iCol, sortorder) {
                if (options.inputOrderBy && options.inputOrderBy.length) {
                    options.inputOrderBy.val("order by"+" "+index + " " + sortorder)
                }
            },
        ...
        if (options.columnModel.length > 0) {
            ...
            options.colModel = [];
            for (var i = 0,
            model; i < options.columnModel.length; i++) {
        ...
        if (options.datatype == "local" && options.rowNum == -1) {
            options.rowNum = options.pageSize;
        }
        ...
        if (options.inputPageNo.length == 0) {
            var pageNo = searchForm.data("no") == undefined? options.initNo:searchForm.data("no");
            var pageSize = searchForm.data("size") == undefined? options.initSize:searchForm.data("size");
            var start = (pageNo-1)*pageSize;
        	searchForm.append('<input id="pageNo" name="pageNo" type="hidden" value="' + (start ? start: "0") + '"/>');
        	searchForm.append('<input id="draw" name="draw" type="hidden" value="' + (pageNo ? pageNo: "0") + '"/>');
            options.inputPageNo = $("#pageNo", searchForm)
        }
        if (options.inputPageSize.length == 0) {
            var pageSize = searchForm.data("size") == undefined? options.initSize:searchForm.data("size");
        	searchForm.append('<input id="pageSize" name="pageSize" type="hidden" value="' + (pageSize ? pageSize: "20") + '"/>');
            options.inputPageSize = $("#pageSize", searchForm)
        }
        if (options.inputOrderBy.length == 0) {
            var orderBy = searchForm.data("order");
            ...
            function() {
                setTimeout(function() {
                    ...
                    if ($.fn.select2 !== undefined) {
                        ...
                        //searchForm.find("select.select2").val("").select2(selectOpt).trigger("change");
                    }
        ...
        if (options.btnSearch.length > 0) {
            options.btnSearch.unbind("click").click(function() {
                ...
                if (searchForm.hasClass("hide")) {
                    ...
                    btnSearch.html(btnSearch.html().replace("显示", "隐藏").replace($.jgrid.extend.btnSearch, $.jgrid.extend.btnHideSearch))
                } else {
                    searchForm.addClass("hide");
                    btnSearch.removeClass("active");
                    btnSearch.html(btnSearch.html().replace("隐藏", "显示").replace($.jgrid.extend.btnHideSearch, $.jgrid.extend.btnSearch))
                }
                resizeDataGrid();
                return false
            });
        ...
        $(window).resizeEnd(function() {
            for (var i in window.dataGrids) {
               /* var gridBox = window.dataGrids[i].parents(".gridBox");
                if(gridBox.length > 0){
                    jqGridExport(c)
                }*/
        ...
        function setGridHeight() {
            ...
            if (typeof options.autoGridHeight == "function") {
                ...
                if (tabContent.length > 0) {
                    ...
                    var gridBox = $(dataGrid).parents(".gridBox");
                    var gridHeader = $(dataGrid).parents(".box-header");
                    var gridPage = $(dataGrid).parents(".gridPage");
                    if(gridBox.length > 0){
                        if(gridPage.length >0){
                            gridPage.height(gridBox.height());
                        }
                        var headerHeight =  gridHeader.length > 0?  gridHeader.height():0;  // box-header 高度
                        var formHeight = searchForm.length >0 ? searchForm.height():0;  // 表单 高度
                        var pageHeight = dataGridPage.length >0 ? dataGridPage.height():0;  //pagination 高度
                        var gridHeader = 43;  //grid 表头高度
                        if (options.groupHeaders && options.groupHeaders.twoLevel) { //当有多级表头时
                            if (options.groupHeaders.threeLevel) {
                                gridHeader *= 3
                            } else {
                                gridHeader *= 2
                        ...
                        gridHeight = ( gridBox.height() - headerHeight - formHeight - pageHeight - options.autoGridHeightFix - gridHeader -70);
                        if (gridHeight < 150) {
                            gridHeight = 150;
                            gridBox.height(150+ headerHeight + formHeight + pageHeight + options.autoGridHeightFix + gridHeader +70);
                            if(gridPage.length >0){
                                gridPage.height(gridBox.height());
                            }
                        }
        ...
        function refresh(pageNo, pageSize) {
            if(options.datatype == 'local' && typeof localReloadGrid == 'function'){
                localReloadGrid(pageNo,pageSize);
                return;
            }
        	if(pageNo == undefined){
        		pageNo = 1;
        		var size = searchForm.data("size");
        ...
        function editGridAddRow(action){
            options.editGridAddRow(action);
        }
        function reloadGrid() {
...
function pageHtmlControl(b) {
    ...
    a.push('<input type="text" style="border:1px solid #367fa9" value="' + b.pageNo + '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
    a.push(b.funcName + "(this.value," + b.pageSize + ",'" + b.funcParam + '\');" onclick="this.select();"/> ' + $.jgrid.extend.pageLabelB + " ");
    a.push('<input type="text" style="border:1px solid #367fa9" value="' + b.pageSize + '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
    ...
    a.jgrid.extend({
        ...
        delTreeChildNode: function(b) {
            return this.each(function() {
                var h = this,
                q = h.p.localReader.id,
                j, g = h.p.treeReader.left_field,
                p = h.p.treeReader.right_field,
                l = h.p.treeReader.expanded_field,
                k = h.p.treeReader.loaded,
                d, e, m, o;
    ...
    a.jgrid.extend({
        setComplexHeaders: function(b) {
            ...
            return this.each(function() {
                ...
                var K = this,
                p = l.defaultStyle === undefined ? true: l.defaultStyle,
                G,
                E,
                n,
                J,
        ...
        createFrozenHtable: function(e, g) {
            ...
            n = a("<tr>", {
                role: "rowheader"
            }).addClass("ui-jqgrid-labels jqg-second-row-header"),
            o = a("<tr>", {
                role: "rowheader"
            }).addClass("ui-jqgrid-labels jqg-third-row-header"),
            O = a("<tr>", {
