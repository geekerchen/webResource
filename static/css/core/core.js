...
if (typeof jQuery === "undefined") {
    throw new Error("Core's JavaScript requires jQuery")
} (function($, window, undefined) {
    $(function() {
        //ccs 由于新增 修改页面body没有占满
        if ($.fn.iCheck !== undefined) {
            $("input[type=checkbox].form-control:not(.noicheck),input[type=radio].form-control:not(.noicheck)").each(function() {
                $(this).iCheck({
                    checkboxClass: "icheckbox_" + ($(this).data("style") || "iradio_square-blue"),
    ...
    var js = {
        ...
        //删除等操作弹出提示窗口
        confirm: function(message, urlOrFun, data, callback, dataType, async, loadingMessage,method) {
        ...
        ajaxSubmitForm: function(formJqueryObj, callback, dataType, async, message) {
            ...
            if(formJqueryObj.find("#token").length === 0){
            	 token = localStorage.getItem("token");
            	 formJqueryObj.append("<input name='token' id='token' value = '"+token+"' type='hidden'>");
            }           
            formJqueryObj.ajaxSubmit($.extend(true, {
                type: formJqueryObj.attr("method"),
                xhrFields: {
        ...
        addTabPage: function($this, title, url, closeable, refresh) {
            ...
            if (!tabPage || typeof tabPage.addTabPage != "function" || (js.cookie("formLayerModel") == "true" && $this && ($this.hasClass("btnTool") || $this.hasClass("btnList"))) || ($this && $this.data("layer") == true)) {
                if (js.layer) {
                    var layerWidth, layerHeight, layerLeft, layerTop,laySize;
                    var size = js.initLayerSize(size);
                    if ($this) {
                        laySize = $this.data("layerSize");
                    ...
                    if (layerWidth == null || layerWidth == "") {
                        if(laySize == "" || laySize == undefined || laySize =="max"){
                            layerWidth = parseInt(size.max[0])+200;
                        }
                        if(laySize == "middle"){
                            layerWidth = parseInt(size.middle[0])+200;
                        }
                        if(laySize == "maxMiddle"){
                            layerWidth = parseInt(size.maxMiddle[0])+200;
                        }
                        if(laySize == "minMiddle"){
                            layerWidth = parseInt(size.minMiddle[0])+200;
                        }
                        if(laySize == "min"){
                            layerWidth = parseInt(size.min[0])+200;
                        }
                    ...
                    if (layerHeight == null || layerHeight == "") {
                        ...
                        if(laySize == "" || laySize == undefined || laySize =="max"){
                            layerHeight = parseInt(size.max[1])+100;
                        }
                        if(laySize == "middle"){
                            layerHeight = parseInt(size.middle[1])+100;
                        }
                        if(laySize == "maxMiddle"){
                            layerHeight = parseInt(size.maxMiddle[1])+100;
                        }
                        if(laySize == "minMiddle"){
                            layerHeight = parseInt(size.minMiddle[1])+100;
                        }
                        if(laySize == "min"){
                            layerHeight = parseInt(size.min[1])+100;
                        }
                    ...
                    js.layer.open({
                        ...
                        success: function(layero, index) {
                            if (layerWidth < 600 || layerHeight < 300) {
                                js.layer.full(index)
                            }
                           // alert(layerWidth);
                           // alert(layerHeight);
        ...
        getCurrentTabPage: function(currentTabCallback) {
          ...
          return  tabPage.getCurrentTabPage(currentTabCallback)
        },
